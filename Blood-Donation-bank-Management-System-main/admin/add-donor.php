...
if(strlen($_SESSION['alogin']) == 0) {	
    ...
    if(isset($_POST['submit'])) {
        ...
        // Handle image upload
        $imagePath = '';
        if(isset($_FILES['donorImage']) && $_FILES['donorImage']['error'] == 0) {
            $image_name = $_FILES['donorImage']['name'];
            $image_loc = $_FILES['donorImage']['tmp_name'];
            $img_des = "img/" . $image_name;
            move_uploaded_file($image_loc, $img_des);
            $imagePath = $img_des;
        }

        $sql = "INSERT INTO tblblooddonars (FullName, MobileNumber, EmailId, Age, Gender, BloodGroup, Address, Message, Status, ImagePath) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ssssssssss", $fullname, $mobile, $email, $age, $gender, $bloodgroup, $address, $message, $status, $imagePath);
        $stmt->execute();
                                    ...
                                    <div class="panel-body">
                                        <form method="post" class="form-horizontal" enctype="multipart/form-data">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="col-sm-4">
                                                    <input type="file" name="donorImage" class="form-control">
                                                </div>
                                            </div>
                                            <div class="form-group">
